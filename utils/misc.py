...
def visualize_KITTI_1(path, data_list, titles = ['input','pred'], cmap=['bwr','autumn'], zdir='y', 
                         xlim=(-1, 1), ylim=(-1, 1), zlim=(-1, 1) ):
    fig = plt.figure(figsize=(6*len(data_list),6))
    cmax = data_list[-1][:,0].max()
    ...
    np.save(os.path.join(path, 'center.npy'), data_list[0].numpy())
    np.save(os.path.join(path, 'coarse_pred.npy'), data_list[1].numpy())
    plt.close(fig)
