...
def TSNE_vis(ret,label,idx):
    ...
    plt.scatter(embedding[:, 0], embedding[:, 1], c=label, cmap=plt.cm.get_cmap('rainbow', 40))
    plt.savefig('/mnt/lustre/feiben/Point_Cloud_Prompt_Tunning/experiments/TSNE/tsne-{}.png'.format(idx))
    plt.close()
...
def test(base_model, test_dataloader, args, config, logger = None):
    ...
    with torch.no_grad():
        for idx, (taxonomy_ids, model_ids, data) in enumerate(test_dataloader):
            ...
            tsne, logits = base_model(points)
            target = label.view(-1)
            tsne_label = tsne.argmax(-1).view(-1)
            pred = logits.argmax(-1).view(-1)
            print('pred.shape',pred.shape, 'tsne_label', tsne_label.shape)
            TSNE_vis(tsne, tsne_label, idx)
