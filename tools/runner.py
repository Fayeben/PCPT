    ...
    with torch.no_grad():
        for idx, (taxonomy_ids, model_ids, data) in enumerate(test_dataloader):
            ...
            if not os.path.exists(data_path):
                ...
                print(dense_points.shape, dense_points[0].shape, points[0].shape)
                misc.visualize_KITTI(
                    os.path.join(data_path, f'{model_ids}_{idx:03d}'),
                    [points[0].cpu(), dense_points[0].cpu()]
                )
                misc.visualize_KITTI_1(
                    os.path.join(data_path, f'{model_ids}_{idx:03d}'),
                    [coarse_points[0].cpu(), center[0].cpu()]
                )
