...
class TransformerEncoder_deep_bak(nn.Module):
    """ Transformer Encoder without hierarchical structure
    """
    def __init__(self, embed_dim=768, depth=4, num_heads=12, mlp_ratio=4., qkv_bias=False, qk_scale=None,
        drop_rate=0., attn_drop_rate=0., drop_path_rate=0.):
...
class PointTransformer_deep_bak(nn.Module):
    def __init__(self, config, **kwargs):
        ...
        for i in range(self.depth):
            self.blocks = TransformerEncoder_deep(
                embed_dim = self.trans_dim,
                depth = self.depth,
                drop_path_rate = dpr[i],
                num_heads = self.num_heads
            )
            self.blocks_list.append(self.blocks)
        ...
        # self.prompt_embed_list = []
        # for i in range(self.depth):
        #     self.prefix_tokens = nn.Parameter(torch.zeros(48, 20, self.trans_dim))
        #     self.prompt_embed = nn.Sequential(
        #         nn.Linear(self.trans_dim, 128),
        #         nn.GELU(),
        #         nn.Linear(128, self.trans_dim)
        #     )
        #     self.prefix_tokens_list.append(self.self.prefix_tokens)
        #     self.prompt_embed_list.append(self.prompt_embed)
    ...
    #     self.prefix_tokens = torch.arange(8).long()
        # self.prefix_encoder = torch.nn.Embedding(8, self.trans_dim)
    
    # def get_prompt(self, batch_size, devices):
    #     prefix_tokens = self.prefix_tokens.unsqueeze(0).expand(batch_size, -1).to(devices)
    #     prompts = self.prefix_encoder(prefix_tokens)
    #     return prefix_tokens, prompts
        self.prefix_tokens_inter_list = []
        for i in range(self.depth):
            self.prefix_tokens_inter = nn.Parameter(torch.zeros(48, 20, self.trans_dim))
            self.prefix_tokens_inter_list.append(self.prefix_tokens_inter)

    def obtain_prompt(self):
        prompt_state_dict = {'conv2': self.conv2.state_dict(),
    ...
    def load_prompt(self, prompt_state_dict):
        self.conv2.load_state_dict(prompt_state_dict['conv2'])
        self.prefix_tokens.load_state_dict(prompt_state_dict['Prompt_Tokens'])
        self.prompt_embed.load_state_dict(prompt_state_dict['Prompt_Embed'])
    ...
    def Freeze(self):
        ...
        self.prompt_embed.requires_grad = True
        for i in range(self.depth):
            self.prefix_tokens_inter_list[i].requires_grad = True
        try:
            for param in self.cls_head_finetune.parameters():
    ...
    def forward(self, pts):
        ...
        # print(cls_tokens.shape)  #[bs,1,384]
        cls_pos = self.cls_pos.expand(group_input_tokens.size(0), -1, -1)  
        # add pos embedding
        # prefix_tokens, prompts = self.get_prompt(batch_size=B, devices=devices)
        # print('prompts.shape', prompts.shape)
        
        prefix_tokens = self.prefix_tokens
        prompts = self.prompt_embed(prefix_tokens)
        ...
        # final input
        x = torch.cat((prefix_tokens, group_input_tokens), dim=1)
        x = torch.cat((cls_tokens, x), dim=1)
        # x = torch.cat((cls_tokens, group_input_tokens), dim=1)
        pos = torch.cat((prompts, pos), dim=1)
        pos = torch.cat((cls_pos, pos), dim=1)
        # pos = torch.cat((cls_pos, pos), dim=1)
        # transformer
        # print('x.shape, pos.shape', x.shape, pos.shape)

        for i in range(self.depth):
            x = self.blocks_list[i](x.to(devices), pos.to(devices))
            # self.prefix_tokens_inter_list[i] = x[x.shape[0],1:21,x.shape[3]]
            # x[x.shape[0],1:21,x.shape[3]] = self.prefix_tokens_inter_list[i]
            x = self.norm(x)
...
@MODELS.register_module()
class Point_BERT_deep_bak(nn.Module):
    ...
    def forward(self, pts, noaug = False, **kwargs):
        if noaug:
            ...
            # divide the point cloud in the same form. This is important
            neighborhood, center = self.group_divider(pts)
            # print('neightbor',neighborhood.shape,'center',center.shape)
