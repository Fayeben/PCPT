...
class TransformerEncoder(nn.Module):
    ...
    def __init__(self, embed_dim=768, depth=4, num_heads=12, mlp_ratio=4., qkv_bias=False, qk_scale=None,
        ...
        for i in range(depth):
            self.prefix_tokens_list.append(nn.Parameter(torch.zeros(16, 100, 384)))
            self.prompt_embed_list.append(nn.Sequential(
                nn.Linear(384, 128),
                nn.GELU(),
                nn.Linear(128, 384)
            ))

    def Freeze_deep(self):
        for i in range(self.depth):
            self.prefix_tokens_list[i].requires_grad = True
            self.prompt_embed_list[i].requires_grad = True

    def forward(self, x, pos):
        for i, block in enumerate(self.blocks):
            x = block(x + pos)
            prefix_tokens = self.prefix_tokens_list[i]
            x[:,1:101,:] = prefix_tokens
        return x

@MODELS.register_module()
class PointTransformer_deep(nn.Module):
    def __init__(self, config, **kwargs):
        ...
        self.prefix_tokens = nn.Parameter(torch.zeros(16, 100, self.trans_dim))
...
@MODELS.register_module()
class Point_BERT_deep(nn.Module):
